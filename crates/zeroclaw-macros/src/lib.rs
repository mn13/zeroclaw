...
pub fn derive_configurable(input: TokenStream) -> TokenStream {
    ...
    for field in fields {
        ...
        // ── Secret handling ──
        //
        // encrypt / decrypt / is_set are dispatched through
        ...
        if is_nested {
            ...
            if let Some(value_ty) = hashmap_value_ty {
                ...
                //
                // Bulk-walk operations (encrypt / decrypt / secret_fields /
                // set_secret), however, only need to iterate the Vec and
                // dispatch on each element's own `T` method — they don't
                // address by name. So those DO push here, mirroring the
