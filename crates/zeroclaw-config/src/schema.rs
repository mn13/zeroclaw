...
pub struct FileUploadConfig {
    ...
    /// `[mcp.servers.*.headers]`. Treated as secret — the values commonly
    /// carry Bearer tokens for the upstream upload endpoint.
    #[serde(default)]
    #[secret]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
    pub headers: HashMap<String, String>,
...
pub struct FileUploadBundleConfig {
    ...
    /// Static HTTP headers attached to every upload request. Treated as
    /// secret — the values commonly carry Bearer tokens for the upstream
    /// upload endpoint.
    #[serde(default)]
    #[secret]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
...
"#;
    ...
    async fn config_save_encrypts_nested_credentials() {
        ...
        // File-upload tool endpoints: headers carry Bearer tokens for the
        // upstream upload service.
        config.file_upload.url = Some("https://uploads.example.invalid/file".into());
        config
            .file_upload
            .headers
            .insert("Authorization".to_string(), "Bearer fu-cred".to_string());
        config.file_upload_bundle.url = Some("https://uploads.example.invalid/bundle".into());
        config
            .file_upload_bundle
            .headers
            .insert("Authorization".to_string(), "Bearer fub-cred".to_string());
        ...
        let webhook = stored.channels.webhook.as_ref().unwrap();
        ...
        // file_upload + file_upload_bundle headers — values encrypted,
        // keys remain plaintext.
        let fu_auth = stored.file_upload.headers.get("Authorization").unwrap();
        assert!(crate::secrets::SecretStore::is_encrypted(fu_auth));
        assert_eq!(store.decrypt(fu_auth).unwrap(), "Bearer fu-cred");
        let fub_auth = stored
            .file_upload_bundle
            .headers
            .get("Authorization")
            .unwrap();
        assert!(crate::secrets::SecretStore::is_encrypted(fub_auth));
        assert_eq!(store.decrypt(fub_auth).unwrap(), "Bearer fub-cred");
